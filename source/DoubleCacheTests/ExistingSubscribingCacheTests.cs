...
{
    public class ExistingSubscribingCacheTests
    {
        ...
        public ExistingSubscribingCacheTests()
        {
            _decoratedCache = A.Fake<ICacheAside>();
            _subscriber = A.Fake<ICacheSubscriber>();
            _subscribingCache = new ExistingItemsSubscribingCache(_decoratedCache, _subscriber);
        }
        ...
        [Fact]
        public void Exists_CallsThrough()
        {
            _subscribingCache.Exists("a");
            A.CallTo(() => _decoratedCache.Exists("a"))
                .MustHaveHappened(Repeated.Exactly.Once);
        }

        [Fact]
        public async Task SubscriberUpdate_ExistingItem_ItemAdded()
        {
            A.CallTo(() => _decoratedCache.Exists("a")).Returns(true);
        ...
        {
            A.CallTo(() => _decoratedCache.Exists("a")).Returns(false);
            A.CallTo(() => _subscriber.GetAsync("a", A<Type>.Ignored)).Returns("b");
            _subscriber.CacheUpdate += Raise.With(this, new CacheUpdateNotificationArgs { Key = "a", Type = typeof(string).AssemblyQualifiedName });
