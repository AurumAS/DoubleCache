...
namespace DoubleCache
{
    public class ExistingItemsSubscribingCache : ICacheAside
    {
        ...
        public ExistingItemsSubscribingCache(ICacheAside cache, ICacheSubscriber cacheSubscriber)
        {
        ...
        private async Task CacheUpdateAction(object sender, CacheUpdateNotificationArgs e)
        {
            ...
            var type = _knownTypes.GetOrAdd(e.Type, Type.GetType(e.Type));
            if (_cache.Exists(e.Key))
            {
                var remoteItem = await _cacheSubscriber.GetAsync(e.Key, type).ConfigureAwait(false); ;
